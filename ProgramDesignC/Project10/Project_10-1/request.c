...
struct request *append_to_list(struct request *list){
 ...
 struct request *new_cur, *new_pre;
...
//loop to order the list by checking if the child enter is before or after the others that already in order by classroom
    ...
    //check if the child enter is already in the list
        ...
        if(current != NULL && strcmp(new_node->classroom, current-> classroom) == 0)
    	{
    		...
    		new_cur = current;
    		new_pre = previous;
            while(new_cur != NULL && strcmp(new_node->classroom, new_cur-> classroom) != 0);
            if(strcmp(new_node->last, new_cur ->last) > 0 )
            {
                new_pre = new_cur;
            ...
            if(new_pre == NULL)//list is empty, the new child enter become the first one in the list
                list = new_node;
            else//new child enter is insert to the list between previous and current
                new_pre-> next = new_node;
            return list;
